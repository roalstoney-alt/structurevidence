#!/usr/bin/env python3
"""Build the StructureEvidence Technical Risk / CML v0.1 domain release."""
...
def schema_files() -> dict[str, dict]:
    ...
    return {
        "evidence/core/schema/evidence_core_record.schema.json": {
            "$schema": "https://json-schema.org/draft/2020-12/schema", "$id": "https://structevidence.com/schema/evidence-core/v0.1", "title": "StructureEvidence Evidence Core Record", "type": "object", "required": core_required,
...
<html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>{html.escape(title)} - StructureEvidence</title><meta name=\"description\" content=\"{html.escape(description)}\"><link rel=\"canonical\" href=\"https://structurevidence.org{html.escape(canonical_path)}\"><link rel=\"icon\" href=\"{prefix}assets/favicon.svg\" type=\"image/svg+xml\"><link rel=\"stylesheet\" href=\"{prefix}assets/technical-risk.css?v=20260916\"></head>
<body><header class=\"tr-header\"><a class=\"tr-brand\" href=\"{prefix}technical-risk/\"><span>SE</span><strong>StructureEvidence</strong><em>Technical Risk</em></a><nav><a href=\"{prefix}technical-risk/search/\">Search</a><a href=\"{prefix}technical-risk/method/\">Method</a><a href=\"{prefix}technical-risk/request-analysis/\" class=\"nav-action\">Request analysis</a></nav></header><main>{body}</main><footer class=\"tr-footer\"><strong>StructureEvidence</strong><span>Evidence for technical supply-chain decisions.</span><a href=\"{prefix}technical-risk/method/\">Method</a><a href=\"{prefix}privacy.html\">Privacy</a><a href=\"{prefix}terms.html\">Terms</a></footer></body></html>"""
...
<section class=\"tr-section intro\"><div><p class=\"eyebrow\">CML / Component Migration &amp; Lifecycle</p><h2>Identify the event. Map the dependency. Test the alternatives. Preserve the evidence.</h2></div><p>StructureEvidence does not create a second evidence database for components. CML is a domain of the same Evidence Core, designed so catalysts, specialty chemicals, APIs, optics, motors and bearings can later reuse the same decision grammar.</p></section>
...
<script src=\"../../assets/cml-search.js?v=20260916\"></script>"""
    return page("Technical record search", "Search StructureEvidence CML public technical records by exact identity.", body, "../../", "/technical-risk/search/")
...
<section class=\"tr-section request-grid\"><div><h2>Include in your request</h2><ul><li>Manufacturer and exact part number</li><li>Lifecycle notice or technical concern</li><li>Assembly or application context</li><li>Decision deadline</li><li>Required verification or qualification scope</li></ul></div><div><h2>Contact</h2><p><strong>Stone Zhu / Structevidence.com</strong></p><p>Hong Kong</p><p><a href=\"https://wa.me/85266629951?text=Hello%20Stone%2C%20I%20would%20like%20to%20request%20a%20StructureEvidence%20Technical%20Risk%20analysis.%20Manufacturer%20and%20part%20number%3A%20\">WhatsApp +852 6662 9951</a></p><p><a href=\"mailto:support@structevidence.com?subject=StructureEvidence%20Technical%20Risk%20request\">support@structevidence.com</a></p><p class=\"note\">Research service only. No brokerage, procurement, custody, exchange or universal substitution warranty.</p></div></section>"""
    return page("Request technical analysis", "Request a scoped StructureEvidence Technical Risk analysis.", body, "../../", "/technical-risk/request-analysis/")


def docs_text(records: list[dict]) -> dict[str, str]:
    architecture = f"""# CML StructureEvidence Integration Assessment v0.1
