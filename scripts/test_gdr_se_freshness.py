...
def main() -> None:
    ...
    status, reason, facts = evaluate_freshness("CORPORATE_TREASURY", "2026-09-08", date(2026, 9, 10), config)
    if status != "UNRESOLVED" or facts["policy_status"] != "UNCONFIGURED":
        fail("UNCONFIGURED freshness policy did not become UNRESOLVED")
    ...
    status, _, facts = evaluate_freshness("L1_NETWORK", "2026-09-01", date(2026, 9, 10), configured)
